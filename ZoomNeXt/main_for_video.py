...
class VideoTestDataset(dataset.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for image_path, mask_path, group_name in clip_info:
            ...
            image_ss.append(jt.array(images[0]).float().divide(255.0).permute(2, 0, 1))
            image_ms.append(jt.array(images[1]).float().divide(255.0).permute(2, 0, 1))
            image_ls.append(jt.array(images[2]).float().divide(255.0).permute(2, 0, 1))

        return dict(
            data={
                "image_s": jt.stack(image_ss, dim=0),
                "image_m": jt.stack(image_ms, dim=0),
                "image_l": jt.stack(image_ls, dim=0),
            },
...
class VideoTrainDataset(dataset.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for image_path, mask_path, _, idx_in_group, _ in self.total_data_paths[index]:
            ...
            image_ms.append(jt.array(images[1]).float().divide(255.0).permute(2, 0, 1))
            image_ls.append(jt.array(images[2]).float().divide(255.0).permute(2, 0, 1))
            masks.append(jt.array(mask).unsqueeze(0))
...
class Evaluator:
    ...
    def eval(self, model, data_loader, save_path=""):
        ...
        for batch in tqdm(data_loader, total=len(data_loader), ncols=79, desc="[EVAL]"):
            ...
            probs = probs.reshape(-1, num_frames, *probs.shape[1:])
            probs = probs.numpy()
...
def test(model, cfg):
    ...
    for te_name in cfg.test.data.names:
        ...
        te_loader = dataset.DataLoader(
            dataset=te_dataset, batch_size=cfg.test.batch_size, num_workers=cfg.test.num_workers
        )
...
def train(model, cfg):
    ...
    counter = recorder.TrainingCounter(
        epoch_length=len(tr_loader)//cfg.train.batch_size,
    ...
    LOGGER.info(f"Image Mean: {model.normalizer.mean.flatten().tolist()}, "
                f"Image Std: {model.normalizer.std.flatten().tolist()}")
    ...
    for _ in range(counter.num_epochs):
        ...
        for batch_idx, batch in enumerate(tr_loader):
            ...
            if cfg.log_interval > 0 and (
                ...
                LOGGER.info(f"{eta_string} | {progress} | {lr_info} | {loss_info} | {data_shape}")
