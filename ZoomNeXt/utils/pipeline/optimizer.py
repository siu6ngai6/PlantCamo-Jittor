...
def group_params(model: nn.Module, group_mode: str, initial_lr: float, optim_cfg: dict):
    if group_mode == "yolov5":
        ...
        params = [
            {"params": list(filter(lambda p: p.requires_grad, bias)), "weight_decay": 0.0},
            {"params": list(filter(lambda p: p.requires_grad, norm)), "weight_decay": 0.0},
            {"params": list(filter(lambda p: p.requires_grad, weight))},
        ]
    elif group_mode == "r3":
        params = [
        ...
        params = [
            {
                "params": list(filter(lambda p: p.requires_grad, params_groups["pretrained"])),
                "lr": optim_cfg.get("diff_factor", 0.1) * initial_lr,
            },
            {
                "params": list(filter(lambda p: p.requires_grad, params_groups["retrained"])),
                "lr": initial_lr,
            },
        ]
        ...
        params = [
            {
                "params": list(filter(lambda p: p.requires_grad, params_groups["pretrained_backbone"])),
                "lr": 0.1 * initial_lr,
            },
            {
                "params": list(filter(lambda p: p.requires_grad, params_groups["pretrained_siamese"])),
                "lr": 0.5 * initial_lr,
            },
            {
                "params": list(filter(lambda p: p.requires_grad, params_groups["retrained"])),
...
if __name__ == "__main__":
    from jittor.models import vgg11_bn
    model = vgg11_bn()
    norm, weight, bias = [], [], []  # 优化器参数分组
    for k, v in model.named_modules():
        if hasattr(v, "bias") and isinstance(v.bias, jt.Var):
            bias.append(v.bias)
        if isinstance(v, nn.BatchNorm):
            norm.append(v.weight)
        elif hasattr(v, "weight") and isinstance(v.weight, jt.Var):
            weight.append(v.weight)

    optimizer = Adam(norm, lr=0.001, betas=(0.98, 0.999))
    optimizer.add_param_group({"params": weight, "weight_decay": 1e-4})
