...
class SimpleASPP(nn.Module):
    ...
    def execute(self, x):
        ...
        # global branch
        y0 = jt.mean(y5, dims=(2, 3), keepdim=True)
...
class DifferenceAwareOps(nn.Module):
    ...
    def __init__(self, num_frames):
        ...
        self.temperal_proj = nn.Sequential(
            nn.Conv(num_frames, num_frames, 3, stride=1, padding=1, bias=False), 
            nn.ReLU(), 
            nn.Conv(num_frames, num_frames, 3, stride=1, padding=1, bias=False)
        )
    ...
    def execute(self, x):
        ...
        shifted_x_tmp = jt.roll(unshifted_x_tmp, shifts=1, dims=-1)
        ...
        attn = jt.nn.softmax(diff_qk, dim=2)
        temperal_diff = jt.linalg.einsum("bxyt,byhwt->bxhwt", attn, diff_v)
...
class RGPU(nn.Module):
    ...
    def __init__(self, in_c, num_groups=6, hidden_dim=None, num_frames=1):
        ...
        self.gate_genator = nn.Sequential(
            ...
            nn.Conv(num_groups * hidden_dim, hidden_dim, 1),
            nn.ReLU(),
            nn.Conv(hidden_dim, num_groups * hidden_dim, 1),
            nn.Softmax(dim=1),
        )
        self.interact = nn.ModuleList()
        self.interact.append(ConvBNReLU(hidden_dim, 3 * hidden_dim, 3, 1, 1))
        for group_id in range(1, num_groups - 1):
            self.interact.append(ConvBNReLU(2 * hidden_dim, 3 * hidden_dim, 3, 1, 1))
        self.interact.append(ConvBNReLU(2 * hidden_dim, 2 * hidden_dim, 3, 1, 1))
    ...
    def execute(self, x):
        ...
        group_id = 0
        curr_x = xs[group_id]
        branch_out = self.interact[group_id](curr_x)
        ...
        gate = self.gate_genator(jt.concat(gates, dim=1))
...
class MHSIU(nn.Module):
    def __init__(self, in_dim, num_groups=4):
        ...
        self.trans = nn.Sequential(
            ...
            nn.Conv(in_dim // num_groups, 3, 1),
            nn.Softmax(dim=1),
        )

    def execute(self, l, m, s):
        tgt_size = tuple(m.shape[2:])

        l = self.conv_l_pre(l)
        l = jt.nn.AdaptiveMaxPool2d(tgt_size)(l) + jt.nn.AdaptiveAvgPool2d(tgt_size)(l)
        s = self.conv_s_pre(s)
        ...
        m = self.conv_m(m)
        s = self.conv_s(s)
        lms = jt.concat([l, m, s], dim=1)  # BT,3C,H,W
