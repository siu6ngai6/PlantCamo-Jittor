...
class ImageTestDataset(dataset.Dataset):
    ...
    def __getitem__(self, index):
        ...
        image_s = jt.array(images[0]).float().divide(255.0).permute(2, 0, 1)
        image_m = jt.array(images[1]).float().divide(255.0).permute(2, 0, 1)
        image_l = jt.array(images[2]).float().divide(255.0).permute(2, 0, 1)
...
class ImageTrainDataset(dataset.Dataset):
    ...
    def __getitem__(self, index):
        ...
        mask = jt.array(mask).unsqueeze(0)
...
class Evaluator():
    ...
    def eval(self, model, data_loader, save_path=''):
        ...
        for batch in tqdm(data_loader, total=len(data_loader), ncols=79, desc='[EVAL]'):
            ...
            probs = logits.sigmoid().squeeze(1).numpy()
