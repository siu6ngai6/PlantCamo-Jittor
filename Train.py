...
def structure_loss(pred, mask):
    ...
    wbce = (weit * wbce).sum(dims=(2, 3)) / weit.sum(dims=(2, 3))
...
def val(model, epoch, save_path, writer):
    ...
    with jt.no_grad():
        ...
        for i in range(test_loader.size):
            ...
            # res, res1 = model(image)
            # eval Dice
            res = nn.upsample( res1[-1], size=gt.shape, mode='bilinear', align_corners=False)
        ...
        writer.add_scalar('MAE', float(mae), global_step=epoch)
        ...
        if epoch == 1:
            ...
            if mae < best_mae:
                best_mae = mae
                best_epoch = epoch
                jt.save(model.state_dict(), save_path + 'Net_epoch_best.pth')
...
def train(train_loader, model, optimizer, epoch, test_path):
    ...
    for i, pack in enumerate(train_loader, start=1):
        for rate in size_rates:
            ...
            if rate != 1:
                images = nn.upsample(images, size=(trainsize, trainsize), mode='bilinear', align_corners=True)
                gts = nn.upsample(gts, size=(trainsize, trainsize), mode='bilinear', align_corners=True)
            # ---- forward ----
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_path', type=str,default='./datasets/train',help='path to train dataset')
    parser.add_argument('--test_path', type=str,default='./datasets/test',help='path to testing dataset')
    ...
    if opt.load is not None:
        pretrained_dict=jt.load(opt.load)
    ...
    if opt.optimizer == 'AdamW':
        optimizer = jt.optim.AdamW(params, opt.lr, weight_decay=1e-4)
    else:
        optimizer = jt.optim.SGD(params, opt.lr, weight_decay=1e-4, momentum=0.9)
    ...
    total_step = len(train_loader)//opt.batchsize
